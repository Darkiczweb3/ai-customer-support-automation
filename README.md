...
The resulting value is an operational confidence score based on model agreement, not a calibrated probability that the classification is correct.

The resulting value is added by the workflow after the model outputs are compared and validated.
...
In the current workflow, LOW messages follow the automatic customer-response path without an additional `CAN_AUTO_REPLY` routing check.
...
The message is logged in Google Sheets before the spam branch is reached, preserving an audit trail while preventing further customer-support routing.
...
| Medium product question with `CAN_AUTO_REPLY=false` | Employee | Passed |
| Low-priority message with `CAN_AUTO_REPLY=true` | Customer | Passed |
...
The generated response can be sent automatically because the message can be handled without access to additional customer, order, payment, delivery, or product-system information.
